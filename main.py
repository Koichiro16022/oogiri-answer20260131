...
def create_geki_video(odai_display, odai_audio, answer_display, answer_audio, video_mode):
    ...
    if video_mode == "縦動画 (9:16)":
        ...
        pos_odai_main = (540, 850)
        pos_odai_sub = (540, 500)
        pos_ans = (540, 850)
    else:
    ...
    try:
        ...
        # 1. お題メイン (i1)
        if odai_len <= 10: odai_main_fontsize = 120
        elif odai_len <= 20: odai_main_fontsize = 100
        elif odai_len <= 30: odai_main_fontsize = 80
        else: odai_main_fontsize = 65

        # 2. お題サブ (i2: 背景パネル用)
        if odai_len <= 15: odai_sub_fontsize = 110
        elif odai_len <= 25: odai_sub_fontsize = 85
        else: odai_sub_fontsize = 55

        # 3. 回答 (i3)
        ans_len = len(clean_ans_disp)
        if ans_len <= 10: ans_fontsize = 120
        elif ans_len <= 20: ans_fontsize = 100
        else: ans_fontsize = 80
        ...
        if os.path.exists(SOUND1): audio_list.append(AudioFileClip(SOUND1).set_start(0.8).volumex(0.2))
        if os.path.exists(SOUND2): audio_list.append(AudioFileClip(SOUND2).set_start(9.0).volumex(0.3))
...
with st.sidebar:
    ...
    with st.form("learning_form", clear_on_submit=True):
        new_odai = st.text_area("お題を追加")
        new_ans = st.text_area("回答を追加")
        new_style = st.selectbox("ユーモアの種類", ["通常", "知的", "ブラック"])
        if st.form_submit_button("感性を覚えさせる"):
            if new_odai and new_ans:
                st.session_state.golden_examples.append({"odai": new_odai, "ans": new_ans, "style": new_style})
                save_data()
                st.rerun()
    
    if st.session_state.golden_examples:
        json_str = json.dumps(st.session_state.golden_examples, ensure_ascii=False, indent=2)
        st.download_button("📥 エクスポート", json_str, file_name=f"learning_data.json", mime="application/json", use_container_width=True)
...
video_mode = st.radio("動画形式を選択してください", mode_options, index=mode_options.index(st.session_state.video_mode_selector), key="new_mode", on_change=on_mode_change, horizontal=True)
...
if st.button("お題生成", use_container_width=True):
    with st.spinner("厳選中..."):
        ...
        r = m.generate_content(f"「{st.session_state.kw}」をテーマにした大喜利お題を3つ作れ。お題だけを3行で出力。")
        st.session_state.odais = [re.sub(r'^[0-9]+[\.．\s]+', '', l).strip() for l in r.text.split('\n') if l.strip()][:3]
        st.rerun()
...
if st.session_state.selected_odai:
    ...
    if st.button("🚀 回答20案生成", type="primary", use_container_width=True):
        with st.spinner("爆笑を追求中..."):
            ...
            ans_raw = [re.sub(r'^[0-9]+[\.．、。\s]+', '', l).strip() for l in r.text.split('\n') if re.match(r'^[0-9]', l)]
